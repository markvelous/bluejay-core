...
const { info, error } = debug_1.getLogger("deployCore");
...
const buildContractFromCache = (cache, net) => {
    const deployments = JSON.parse(fs_1.readFileSync(cache).toString())[net];
...
exports.deployCore = async () => {
    ...
    // Fund new wallet
    await deployer.sendTransaction({ to: account1.address, value: utils_1.exp(18) });
    // Mint some 1k USD for account
    await collateral.mint(account1.address, utils_1.exp(18).mul(1000));
    ...
    await collateralJoin
        .connect(account1)
        .deposit(account1.address, utils_1.exp(18).mul(1000), transactionOverrides);
    //   Draw out 1m of MMK
    await ledger
        .connect(account1)
        .modifyPositionCollateralization(collateralType, account1.address, account1.address, account1.address, utils_1.exp(18).mul(1000), utils_1.exp(18).mul(1000 * 1000), transactionOverrides);
